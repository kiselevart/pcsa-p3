...
void process_instruction()
{
    ...
    if (op == 0) { //R-TYPE instructions
        ...
        if (op == ADDI) { 
            ...
            printf("LB\n");
            int extended = imm;
            int vAddr = CURRENT_STATE.REGS[rs] + extended;
            int8_t data = mem_read_32(vAddr);
            CURRENT_STATE.REGS[rt] = (int)data;
            NEXT_STATE.PC = CURRENT_STATE.PC + 4; 
        }
        else if (op == LBU) { 
            printf("LBU\n");
            int extended = imm;
            int vAddr = CURRENT_STATE.REGS[rs] + extended;
            NEXT_STATE.REGS[rt] = (0xFF & mem_read_32(vAddr));
            NEXT_STATE.PC = CURRENT_STATE.PC + 4; 
        }
        else if (op == LH) {
            printf("LH\n");
            ...
            int extended = imm;
            int vAddr = CURRENT_STATE.REGS[rs] + extended;
            NEXT_STATE.REGS[rt] = (0xFFFF & mem_read_32(vAddr));
            NEXT_STATE.PC = CURRENT_STATE.PC + 4; 
        }
        else if (op == LUI) { 
            printf("LUI\n");
            ...
            int extended = imm;
            int vAddr = CURRENT_STATE.REGS[rs] + extended;
            mem_write_32(vAddr, (0xFF & CURRENT_STATE.REGS[rt]));
            NEXT_STATE.PC = CURRENT_STATE.PC + 4; 
        }
        else if (op == SH) {
            printf("SH\n");
            int extended = imm;
            int vAddr = CURRENT_STATE.REGS[rs] + extended;
            mem_write_32(vAddr, (0xFFFF & CURRENT_STATE.REGS[rt]));
            NEXT_STATE.PC = CURRENT_STATE.PC + 4; 
        }
        else if (op == SLTI) { 
            printf("SLTI\n");
